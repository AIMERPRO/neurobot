...
async def pay_handler(callback: CallbackQuery):
    ...
    link_for_pay = (f"https://payanyway.ru/assistant.htm?MNT_ID=secret&MNT_AMOUNT={settings.oneday_subs}"
...
async def pay_handler(callback: CallbackQuery):
    ...
    link_for_pay = (f"https://payanyway.ru/assistant.htm?MNT_ID=secret&MNT_AMOUNT={settings.sevenday_subs}"
                    f"&MNT_TRANSACTION_ID=sub_{payment.id}_{order_uuid}"
                    f"&MNT_CURRENCY_CODE=RUB&MNT_TEST_MODE=0&MNT_SUBSCRIBER_ID={callback.from_user.id}")
...
async def pay_handler(callback: CallbackQuery):
    ...
    link_for_pay = (f"https://payanyway.ru/assistant.htm?MNT_ID=secret&MNT_AMOUNT={settings.month_subs}"
                    f"&MNT_TRANSACTION_ID=sub_{payment.id}_{order_uuid}"
                    f"&MNT_CURRENCY_CODE=RUB&MNT_TEST_MODE=0&MNT_SUBSCRIBER_ID={callback.from_user.id}")
